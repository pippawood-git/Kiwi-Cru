...
class CartItems extends HTMLElement {
  constructor() {
    super();
    this.cart = null;
    this.cartBtn = document.querySelector('.site-header__link.btn--icon[aria-label="Cart"]');
  ...
  connectedCallback() {
    ...
    window.FoxThemeEvents.subscribe('ON_CART_UPDATED', (cart) => {
      ...
      this.cartBtn.classList.toggle('is-empty', cart.item_count === 0);
    });

    window.FoxThemeEvents.subscribe('ON_ITEM_ADDED', (cart) => {
      this.classList.toggle('is-empty', cart.item_count === 0);
      this.cartBtn.classList.toggle('is-empty', cart.item_count === 0);
    });
  }
